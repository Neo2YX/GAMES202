        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    // Edit here
                    float delta_omega = CalcArea(x, y, width, height);

                    for (int l = 0; l <= SHOrder; ++l) {
                        for(int m = -l; m <= l; m++){
                            float shBasis = sh::EvalSH(l, m, Eigen::Vector3d(dir[0], dir[1], dir[2]).normalized());  
                            //int shIndex = l * l + m + l + 1;
                            int shIndex = sh::GetIndex(l, m);
                            SHCoeffiecents[shIndex] += shBasis * delta_omega * Le;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            for (int i = 0; i < mesh->getVertexCount(); ++i)
            {
                const Eigen::Vector3f& v = mesh->getVertexPositions().col(i);
                const Eigen::Vector3f& n = mesh->getVertexNormals().col(i);
                auto indirectLightCoeff = CalcInterReflection(mesh, scene, v, n, SHCoeffLength, 1);

                for (int j = 0; j < indirectLightCoeff->size(); j++)
                {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*indirectLightCoeff)[j]/ M_PI;
                }
    ...
    std::unique_ptr<std::vector<double>> CalcInterReflection(const Mesh* mesh, const Scene* scene, const Eigen::Vector3f& p, const Eigen::Vector3f& n, const int& SHCoeffNUM, int bounces)
    {
        std::unique_ptr<std::vector<double>> coeff(new std::vector<double>());
        coeff->assign(SHCoeffNUM, 0.0);

        if (m_Bounce < bounces) return coeff;
        ...
        for (int t = 0; t < sample_side; t++) {
            for (int p = 0; p < sample_side; p++) {
                ...
                Eigen::Vector3f wi = sh::ToVector(phi, theta).cast<float>().normalized();
                Eigen::Vector3f normal = n.normalized();
                float H = wi.dot(normal);

                Intersection its;
                if (H > 0.0 && scene->rayIntersect(Ray3f(p, wi), its))
                {
                    sampleCnt++;
                    Vector3f index = its.tri_index;
                    for (int i = 0; i < SHCoeffNUM; ++i)
                    {
                        (*coeff)[i] = m_TransportSHCoeffs.col(index.x()).coeffRef(i) * its.bary.x() + m_TransportSHCoeffs.col(index.y()).coeffRef(i) * its.bary.y() + m_TransportSHCoeffs.col(index.z()).coeffRef(i) * its.bary.z();
                    }
                    Vector3f interNormal = mesh->getVertexNormals().col(index.x()) * its.bary.x()
                        + mesh->getVertexNormals().col(index.y()) * its.bary.y()
                        + mesh->getVertexNormals().col(index.y()) * its.bary.y();
                    auto indCoeff = CalcInterReflection(mesh, scene, its.p, interNormal, SHCoeffNUM, bounces + 1);
                    for (int i = 0; i < SHCoeffNUM; ++i)
                    {
                        (*coeff)[i] += (*indCoeff)[i];
                        (*coeff)[i] *= H;
        ...
        for (unsigned int i = 0; i < coeff->size(); i++) {
            //(*coeff)[i] /= (sample_side*sample_side);
            if (sampleCnt) (*coeff)[i] /= sampleCnt;
        }
        return coeff;
    }
